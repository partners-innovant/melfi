...
serve(async (req) => {
  ...
  try {
    const { action = "search", query, onlyPdf = true, minCitations, yearFrom, yearTo } = await req.json();
    ...
    if (!res.ok) {
      ...
      return new Response(JSON.stringify({ articles: [], error: `EuropePMC error: ${res.status}` }), {
        headers: corsHeaders,
      });
    }
    ...
    const articles = (data.resultList?.result || []).map((a: any) => {
      ...
      return {
        ...
        citations_count:
          typeof a.citedByCount === "number" ? a.citedByCount : a.citedByCount ? Number(a.citedByCount) : null,
      };
    });
